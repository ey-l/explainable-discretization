    ...
    def _select_node(self):
        ...
        while True:
            if current_node.clusterID is not None:
                self._explore_one_node_in_list(self.clusters[current_node.clusterID])
                break
            if current_node.count == 0:
                ls = self._get_all_successors(current_node)
                ls = [n.ID for n in ls]
                self._explore_one_node_in_list(ls)
                break
...
if __name__ == '__main__':
    ...
    for round in range(20):
        ...
        # Initialize UCB
        dendrogram.initialize_ucb(n_samples=10)
        for i in range(6):
            print(f"Round {i}")
            print(dendrogram._select_node())
